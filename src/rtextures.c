...
*
*   raylib-on-vulkan NOTES:
*     - All GPU access goes through the rlgl-compatible layer in rlvk.h (Vulkan), no OpenGL
*     - This module owns the implementations of: stb_image, stb_image_write,
*       stb_image_resize2, stb_perlin, qoi and rltexgpu (see CONVENTIONS.md)
*     - No contract extension was required: only functions declared in rlvk.h are used
*
*   CONFIGURATION:
...
#include "rlvk.h"               // rlgl-compatible immediate-mode layer implemented on Vulkan
...
static void ImageBlit(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint); // Axis-aligned image blit with scaling, used by all ImageDrawImage*()
    ...
    for (int i = 0; i < textLength;)
    {
        ...
        else
        {
            if ((codepoint != ' ') && (codepoint != '\t'))
            {
                ...
                ImageBlit(&imText, font.glyphs[index].image, (Rectangle){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
        ...
        for (int i = 0, k = 0; i < image.width*image.height; i++)
        {
            ...
            switch (image.format)
            {
                ...
                case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
                {
                    ...
                    if (selectedChannel == 0) pixelValue = (float)((pixel & 0xf800) >> 11)*(1.0f/31);
                    else if (selectedChannel == 1) pixelValue = (float)((pixel & 0x07c0) >> 6)*(1.0f/31);
                    else if (selectedChannel == 2) pixelValue = (float)((pixel & 0x003e) >> 1)*(1.0f/31);
                    else if (selectedChannel == 3) pixelValue = ((pixel & 0x0001) == 0)? 0.0f : 1.0f;
                ...
                case PIXELFORMAT_UNCOMPRESSED_R5G6B5:
                {
                    ...
                    if (selectedChannel == 0) pixelValue = (float)((pixel & 0xf800) >> 11)*(1.0f/31);
                    else if (selectedChannel == 1) pixelValue = (float)((pixel & 0x07e0) >> 5)*(1.0f/63);
                    else if (selectedChannel == 2) pixelValue = (float)(pixel & 0x001f)*(1.0f/31);
                ...
                case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4:
                {
                    ...
                    if (selectedChannel == 0) pixelValue = (float)((pixel & 0xf000) >> 12)*(1.0f/15);
                    else if (selectedChannel == 1) pixelValue = (float)((pixel & 0x0f00) >> 8)*(1.0f/15);
                    else if (selectedChannel == 2) pixelValue = (float)((pixel & 0x00f0) >> 4)*(1.0f/15);
                    else if (selectedChannel == 3) pixelValue = (float)(pixel & 0x000f)*(1.0f/15);
    ...
    else
    {
        switch (image->format)
        {
            ...
            case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
            {
                ...
                for (int i = 0; i < image->width*image->height; i++)
                {
                    if ((((unsigned short *)image->data)[i] & 0x0001) <= thresholdValue)
                    {
                        ((unsigned short *)image->data)[i] = (unsigned short)r << 11 | (unsigned short)g << 6 | (unsigned short)b << 1 | (unsigned short)a;
                    }
                }
...
void ImageColorContrast(Image *image, int contrastValue)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    float contrast = (100.0f + (float)contrastValue)/100.0f;
    contrast *= contrast;
        ...
        for (int i = 0, k = 0; i < image.width*image.height; i++)
        {
            switch (image.format)
            {
                ...
                case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
                {
                    ...
                    pixels[i].r = (unsigned char)((float)((pixel & 0xf800) >> 11)*(255/31));
                    pixels[i].g = (unsigned char)((float)((pixel & 0x07c0) >> 6)*(255/31));
                    pixels[i].b = (unsigned char)((float)((pixel & 0x003e) >> 1)*(255/31));
                    pixels[i].a = (unsigned char)((pixel & 0x0001)*255);
                ...
                case PIXELFORMAT_UNCOMPRESSED_R5G6B5:
                {
                    ...
                    pixels[i].r = (unsigned char)((float)((pixel & 0xf800) >> 11)*(255/31));
                    pixels[i].g = (unsigned char)((float)((pixel & 0x07e0) >> 5)*(255/63));
                    pixels[i].b = (unsigned char)((float)(pixel & 0x001f)*(255/31));
                    pixels[i].a = 255;
                ...
                case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4:
                {
                    ...
                    pixels[i].r = (unsigned char)((float)((pixel & 0xf000) >> 12)*(255/15));
                    pixels[i].g = (unsigned char)((float)((pixel & 0x0f00) >> 8)*(255/15));
                    pixels[i].b = (unsigned char)((float)((pixel & 0x00f0) >> 4)*(255/15));
                    pixels[i].a = (unsigned char)((float)(pixel & 0x000f)*(255/15));
    ...
    if ((x >=0) && (x < image.width) && (y >= 0) && (y < image.height))
    {
        switch (image.format)
        {
            ...
            case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
            {
                ...
                color.r = (unsigned char)((float)((pixel & 0xf800) >> 11)*(255/31));
                color.g = (unsigned char)((float)((pixel & 0x07c0) >> 6)*(255/31));
                color.b = (unsigned char)((float)((pixel & 0x003e) >> 1)*(255/31));
                color.a = (unsigned char)((pixel & 0x0001)*255);
            ...
            case PIXELFORMAT_UNCOMPRESSED_R5G6B5:
            {
                ...
                color.r = (unsigned char)((float)((pixel & 0xf800) >> 11)*(255/31));
                color.g = (unsigned char)((float)((pixel & 0x07e0) >> 5)*(255/63));
                color.b = (unsigned char)((float)(pixel & 0x001f)*(255/31));
                color.a = 255;
            ...
            case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4:
            {
                ...
                color.r = (unsigned char)((float)((pixel & 0xf000) >> 12)*(255/15));
                color.g = (unsigned char)((float)((pixel & 0x0f00) >> 8)*(255/15));
                color.b = (unsigned char)((float)((pixel & 0x00f0) >> 4)*(255/15));
                color.a = (unsigned char)((float)(pixel & 0x000f)*(255/15));
...
void ImageDrawLine(Image *dst, int startPosX, int startPosY, int endPosX, int endPosY, Color color)
{
    ...
    // NOTE: Multiplication instead of a left shift, shifting a negative value is undefined behaviour
    int decInc = (longLen == 0)? 0 : (int)(((long long)shortLen*65536)/longLen);
...
void ImageDrawLineStrip(Image *dst, const Vector2 *points, int pointCount, Color color)
{
    ...
    for (int i = 0; i < (pointCount - 1); i++) ImageDrawLineV(dst, points[i], points[i + 1], color);
}
...
// NOTE: Color is interpolated from inner (center) to outer (border) on every pixel
void ImageDrawCircleGradient(Image *dst, Vector2 center, float radius, Color inner, Color outer)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0) || (radius <= 0.0f)) return;

    int xMin = (int)floorf(center.x - radius);
    int yMin = (int)floorf(center.y - radius);
    int xMax = (int)ceilf(center.x + radius);
    int yMax = (int)ceilf(center.y + radius);
    ...
    for (int y = yMin; y < yMax; y++)
    {
        for (int x = xMin; x < xMax; x++)
        {
            ...
            ImageDrawPixel(dst, x, y, ColorLerp(inner, outer, distance/radius));
        }
    }
}
...
{
    ImageDrawRectangleLinesEx(dst, (Rectangle){ (float)posX, (float)posY, (float)width, (float)height }, 1, color);
}

// Draw rectangle lines within an image with line thickness
// NOTE: Lines are drawn inside the provided rectangle
void ImageDrawRectangleLinesEx(Image *dst, Rectangle rec, int thick, Color color)
{
    if (thick <= 0) return;

    // Clamp thickness to rectangle size, a thicker line just fills the rectangle
    if ((float)thick*2.0f > rec.width) thick = (int)(rec.width/2.0f);
    if ((float)thick*2.0f > rec.height) thick = (int)(rec.height/2.0f);
    if (thick <= 0) thick = 1;
...
// Draw a color-filled rectangle with pro parameters within an image
// NOTE: origin is defined in rectangle space, rotation is applied around it (degrees)
void ImageDrawRectanglePro(Image *dst, Rectangle rec, Vector2 origin, float rotation, Color color)
{
    ...
    if (rotation == 0.0f)
    {
        ImageDrawRectangleRec(dst, (Rectangle){ rec.x - origin.x, rec.y - origin.y, rec.width, rec.height }, color);
        return;
    }
    ...
    // Rotated rectangle corners, used to compute the scanned bounding box
    const float dx[4] = { -origin.x, rec.width - origin.x, rec.width - origin.x, -origin.x };
    const float dy[4] = { -origin.y, -origin.y, rec.height - origin.y, rec.height - origin.y };

    float minX = 0.0f, maxX = 0.0f, minY = 0.0f, maxY = 0.0f;

    for (int i = 0; i < 4; i++)
    {
        float cx = rec.x + dx[i]*cosRotation - dy[i]*sinRotation;
        float cy = rec.y + dx[i]*sinRotation + dy[i]*cosRotation;

        if ((i == 0) || (cx < minX)) minX = cx;
        if ((i == 0) || (cx > maxX)) maxX = cx;
        if ((i == 0) || (cy < minY)) minY = cy;
        if ((i == 0) || (cy > maxY)) maxY = cy;
    }
    ...
    for (int y = yMin; y < yMax; y++)
    {
        for (int x = xMin; x < xMax; x++)
        {
            float px = ((float)x + 0.5f) - rec.x;
            float py = ((float)y + 0.5f) - rec.y;

            float lx = px*cosRotation + py*sinRotation + origin.x;
            float ly = -px*sinRotation + py*cosRotation + origin.y;

            if ((lx < 0.0f) || (lx >= rec.width) || (ly < 0.0f) || (ly >= rec.height)) continue;
...
// NOTE: Colors order: top-left, bottom-left, bottom-right, top-right
void ImageDrawRectangleGradientEx(Image *dst, Rectangle rec, Color col1, Color col2, Color col3, Color col4)
{
    ...
    int xMin = (int)rec.x;
    int yMin = (int)rec.y;
    int xMax = (int)(rec.x + rec.width);
    int yMax = (int)(rec.y + rec.height);

    int clampedXMin = (xMin < 0)? 0 : xMin;
    int clampedYMin = (yMin < 0)? 0 : yMin;
    int clampedXMax = (xMax > dst->width)? dst->width : xMax;
    int clampedYMax = (yMax > dst->height)? dst->height : yMax;

    float invWidth = (xMax > xMin)? 1.0f/(float)(xMax - xMin) : 0.0f;
    float invHeight = (yMax > yMin)? 1.0f/(float)(yMax - yMin) : 0.0f;

    for (int y = clampedYMin; y < clampedYMax; y++)
    {
        float factorY = (float)(y - yMin)*invHeight;

        // Bilinear interpolation: left edge (col1 -> col2), right edge (col4 -> col3)
        Color left = ColorLerp(col1, col2, factorY);
        Color right = ColorLerp(col4, col3, factorY);

        for (int x = clampedXMin; x < clampedXMax; x++)
        {
            float factorX = (float)(x - xMin)*invWidth;

            ImageDrawPixel(dst, x, y, ColorLerp(left, right, factorX));
        }
    }
}
...
// Draw an image (source) within an image (destination), axis-aligned, with scaling
// NOTE: Color tint is applied to source image
static void ImageBlit(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint)
{
    // Security check to avoid program crash
    ...
    else
    {
        ...
        if ((dst->mipmaps > 1) && (src.mipmaps > 1))
        {
            ...
            ImageBlit(&mipmapDst, mipmapSrc, mipmapSrcRec, mipmapDstRec, tint);
        }
    }
}
...
{
    ImageBlit(dst, src, (Rectangle){ 0.0f, 0.0f, (float)src.width, (float)src.height },
        (Rectangle){ (float)posX, (float)posY, (float)src.width, (float)src.height }, tint);
}
...
{
    ImageBlit(dst, src, srcRec, (Rectangle){ position.x, position.y, fabsf(srcRec.width), fabsf(srcRec.height) }, tint);
}

// Draw a part of an image defined by a rectangle into a destination rectangle,
// with scaling and rotation, within an image
// NOTE: origin is defined in destination rectangle space, rotation is applied around it (degrees)
void ImageDrawImagePro(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Vector2 origin, float rotation, Color tint)
{
    ...
    {
        TRACELOG(LOG_WARNING, "IMAGE: Image drawing not supported for compressed formats");
        return;
    }

    if ((dstRec.width == 0.0f) || (dstRec.height == 0.0f)) return;

    // Fast path: no rotation required, plain (scaled) blit
    if (rotation == 0.0f)
    {
        dstRec.x -= origin.x;
        dstRec.y -= origin.y;
        ImageBlit(dst, src, srcRec, dstRec, tint);
        return;
    }

    // Source rectangle out-of-bounds security checks, flipping is supported through negative sizes
    bool flipX = false;
    bool flipY = false;

    if (srcRec.width < 0) { flipX = true; srcRec.width *= -1; srcRec.x -= srcRec.width; }
    if (srcRec.height < 0) { flipY = true; srcRec.height *= -1; srcRec.y -= srcRec.height; }
    ...
    // Rotated destination quad, defined around dstRec position (rotation pivot)
    float sinRotation = sinf(rotation*DEG2RAD);
    float cosRotation = cosf(rotation*DEG2RAD);

    // Corners of the destination quad, in image space
    Vector2 corner[4] = { 0 };
    const float dx[4] = { -origin.x, dstRec.width - origin.x, dstRec.width - origin.x, -origin.x };
    const float dy[4] = { -origin.y, -origin.y, dstRec.height - origin.y, dstRec.height - origin.y };

    for (int i = 0; i < 4; i++)
    {
        corner[i].x = dstRec.x + dx[i]*cosRotation - dy[i]*sinRotation;
        corner[i].y = dstRec.y + dx[i]*sinRotation + dy[i]*cosRotation;
    }

    // Bounding box of the rotated quad, clamped to destination image
    float minX = corner[0].x, maxX = corner[0].x;
    float minY = corner[0].y, maxY = corner[0].y;
    ...
    int bytesPerPixelDst = GetPixelDataSize(1, 1, dst->format);
    int bytesPerPixelSrc = GetPixelDataSize(1, 1, src.format);
    int strideDst = GetPixelDataSize(dst->width, 1, dst->format);
    int strideSrc = GetPixelDataSize(src.width, 1, src.format);

    // Inverse-map every destination pixel into source space (nearest-neighbour sampling)
    for (int y = yMin; y < yMax; y++)
    {
        for (int x = xMin; x < xMax; x++)
        {
            // Undo translation and rotation to get local destination-rectangle coordinates
            float px = ((float)x + 0.5f) - dstRec.x;
            float py = ((float)y + 0.5f) - dstRec.y;

            float lx = px*cosRotation + py*sinRotation + origin.x;
            float ly = -px*sinRotation + py*cosRotation + origin.y;

            if ((lx < 0.0f) || (lx >= dstRec.width) || (ly < 0.0f) || (ly >= dstRec.height)) continue;

            float u = lx/dstRec.width;
            float v = ly/dstRec.height;

            if (flipX) u = 1.0f - u;
            if (flipY) v = 1.0f - v;

            int sx = (int)(srcRec.x + u*srcRec.width);
            int sy = (int)(srcRec.y + v*srcRec.height);
            ...
            unsigned char *pSrc = (unsigned char *)src.data + sy*strideSrc + sx*bytesPerPixelSrc;
            unsigned char *pDst = (unsigned char *)dst->data + y*strideDst + x*bytesPerPixelDst;

            Color colSrc = GetPixelColor(pSrc, src.format);
            Color colDst = GetPixelColor(pDst, dst->format);

            SetPixelColor(pDst, ColorAlphaBlend(colDst, colSrc, tint), dst->format);
        }
    }
}
...
{
    ImageDrawImagePro(dst, src, (Rectangle){ 0.0f, 0.0f, (float)src.width, (float)src.height },
        (Rectangle){ position.x, position.y, (float)src.width*scale, (float)src.height*scale },
        (Vector2){ 0.0f, 0.0f }, rotation, tint);
}
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
    ...
    ImageBlit(dst, imText, srcRec, dstRec, WHITE);

    UnloadImage(imText);
}

// Draw text using Font and pro parameters (rotation) within an image (destination)
void ImageDrawTextPro(Image *dst, Font font, const char *text, Vector2 position, Vector2 origin, float rotation, float fontSize, float spacing, Color tint)
{
    ...
    ImageDrawImagePro(dst, imText, srcRec, dstRec, origin, rotation, WHITE);
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++) ImageBlit(&faces, mipmapped, faceRecs[i], (Rectangle){ 0, (float)size*i, (float)size, (float)size }, WHITE);
...
Color GetPixelColor(const void *srcPtr, int format)
{
    ...
    switch (format)
    {
        case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: color = (Color){ ((const unsigned char *)srcPtr)[0], ((const unsigned char *)srcPtr)[0], ((const unsigned char *)srcPtr)[0], 255 }; break;
        case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: color = (Color){ ((const unsigned char *)srcPtr)[0], ((const unsigned char *)srcPtr)[0], ((const unsigned char *)srcPtr)[0], ((const unsigned char *)srcPtr)[1] }; break;
        case PIXELFORMAT_UNCOMPRESSED_R5G6B5:
        {
            color.r = (unsigned char)((((const unsigned short *)srcPtr)[0] >> 11)*255/31);
            color.g = (unsigned char)(((((const unsigned short *)srcPtr)[0] >> 5) & 0x003f)*255/63);
            color.b = (unsigned char)((((const unsigned short *)srcPtr)[0] & 0x001f)*255/31);
            color.a = 255;
        ...
        {
            color.r = (unsigned char)((((const unsigned short *)srcPtr)[0] >> 11)*255/31);
            color.g = (unsigned char)(((((const unsigned short *)srcPtr)[0] >> 6) & 0x001f)*255/31);
            color.b = (unsigned char)((((const unsigned short *)srcPtr)[0] & 0x001f)*255/31);
            color.a = (((const unsigned short *)srcPtr)[0] & 0x0001)? 255 : 0;

        } break;
        case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4:
        {
            color.r = (unsigned char)((((const unsigned short *)srcPtr)[0] >> 12)*255/15);
            color.g = (unsigned char)(((((const unsigned short *)srcPtr)[0] >> 8) & 0x000f)*255/15);
            color.b = (unsigned char)(((((const unsigned short *)srcPtr)[0] >> 4) & 0x000f)*255/15);
            color.a = (unsigned char)((((const unsigned short *)srcPtr)[0] & 0x000f)*255/15);

        } break;
        case PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: color = (Color){ ((const unsigned char *)srcPtr)[0], ((const unsigned char *)srcPtr)[1], ((const unsigned char *)srcPtr)[2], ((const unsigned char *)srcPtr)[3] }; break;
        case PIXELFORMAT_UNCOMPRESSED_R8G8B8: color = (Color){ ((const unsigned char *)srcPtr)[0], ((const unsigned char *)srcPtr)[1], ((const unsigned char *)srcPtr)[2], 255 }; break;
        case PIXELFORMAT_UNCOMPRESSED_R32:
        {
            // NOTE: Pixel normalized float value is converted to [0..255]
            color.r = (unsigned char)(((const float *)srcPtr)[0]*255.0f);
            color.g = (unsigned char)(((const float *)srcPtr)[0]*255.0f);
            color.b = (unsigned char)(((const float *)srcPtr)[0]*255.0f);
            color.a = 255;
        ...
        case PIXELFORMAT_UNCOMPRESSED_R32G32B32:
        {
            // NOTE: Pixel normalized float value is converted to [0..255]
            color.r = (unsigned char)(((const float *)srcPtr)[0]*255.0f);
            color.g = (unsigned char)(((const float *)srcPtr)[1]*255.0f);
            color.b = (unsigned char)(((const float *)srcPtr)[2]*255.0f);
            color.a = 255;
        ...
        case PIXELFORMAT_UNCOMPRESSED_R32G32B32A32:
        {
            ...
            color.g = (unsigned char)(((const float *)srcPtr)[1]*255.0f);
            color.b = (unsigned char)(((const float *)srcPtr)[2]*255.0f);
            color.a = (unsigned char)(((const float *)srcPtr)[3]*255.0f);

        } break;
        case PIXELFORMAT_UNCOMPRESSED_R16:
        {
            // NOTE: Pixel normalized float value is converted to [0..255]
            color.r = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[0])*255.0f);
            color.g = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[0])*255.0f);
            color.b = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[0])*255.0f);
            color.a = 255;
        ...
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16:
        {
            // NOTE: Pixel normalized float value is converted to [0..255]
            color.r = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[0])*255.0f);
            color.g = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[1])*255.0f);
            color.b = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[2])*255.0f);
            color.a = 255;
        ...
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
        {
            ...
            color.g = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[1])*255.0f);
            color.b = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[2])*255.0f);
            color.a = (unsigned char)(HalfToFloat(((const unsigned short *)srcPtr)[3])*255.0f);
...
void SetPixelColor(void *dstPtr, Color color, int format)
{
    switch (format)
    {
        ...
        // NOTE: Float/half formats are also supported for writing, so image software
        // rendering (ImageDraw*()) works on HDR images as it does on 8-bit ones
        case PIXELFORMAT_UNCOMPRESSED_R32:
        {
            // NOTE: Calculate grayscale equivalent color (normalized to 32bit float)
            Vector3 coln = { (float)color.r/255.0f, (float)color.g/255.0f, (float)color.b/255.0f };

            ((float *)dstPtr)[0] = coln.x*0.299f + coln.y*0.587f + coln.z*0.114f;

        } break;
        case PIXELFORMAT_UNCOMPRESSED_R32G32B32:
        {
            ((float *)dstPtr)[0] = (float)color.r/255.0f;
            ((float *)dstPtr)[1] = (float)color.g/255.0f;
            ((float *)dstPtr)[2] = (float)color.b/255.0f;
        ...
        case PIXELFORMAT_UNCOMPRESSED_R32G32B32A32:
        {
            ...
            ((float *)dstPtr)[1] = (float)color.g/255.0f;
            ((float *)dstPtr)[2] = (float)color.b/255.0f;
            ((float *)dstPtr)[3] = (float)color.a/255.0f;

        } break;
        case PIXELFORMAT_UNCOMPRESSED_R16:
        {
            // NOTE: Calculate grayscale equivalent color (normalized to 16bit float)
            Vector3 coln = { (float)color.r/255.0f, (float)color.g/255.0f, (float)color.b/255.0f };

            ((unsigned short *)dstPtr)[0] = FloatToHalf(coln.x*0.299f + coln.y*0.587f + coln.z*0.114f);

        } break;
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16:
        {
            ((unsigned short *)dstPtr)[0] = FloatToHalf((float)color.r/255.0f);
            ((unsigned short *)dstPtr)[1] = FloatToHalf((float)color.g/255.0f);
            ((unsigned short *)dstPtr)[2] = FloatToHalf((float)color.b/255.0f);
        ...
        case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
        {
            ...
            ((unsigned short *)dstPtr)[1] = FloatToHalf((float)color.g/255.0f);
            ((unsigned short *)dstPtr)[2] = FloatToHalf((float)color.b/255.0f);
            ((unsigned short *)dstPtr)[3] = FloatToHalf((float)color.a/255.0f);
...
static float HalfToFloat(unsigned short x)
{
    ...
    // NOTE: Every term is guarded instead of multiplied by a condition,
    // the shift amounts are only well-defined inside their own branch
    unsigned int normalized = 0;
    unsigned int denormalized = 0;

    if (e != 0) normalized = ((e + 112) << 23) | m;
    else if (m != 0) denormalized = ((v - 37) << 23) | ((m << (150 - v)) & 0x007fe000);

    uni.ui = ((unsigned int)(x & 0x8000) << 16) | normalized | denormalized; // sign : normalized : denormalized
...
static unsigned short FloatToHalf(float x)
{
    ...
    unsigned int normalized = 0;
    unsigned int denormalized = 0;
    unsigned int saturate = 0;

    if (e > 112) normalized = ((((e - 112) << 10) & 0x7c00) | (m >> 13));
    else if ((e < 113) && (e > 101)) denormalized = ((((0x007ff000 + m) >> (125 - e)) + 1) >> 1);

    if (e > 143) saturate = 0x7fff;

    result = (unsigned short)(((b & 0x80000000) >> 16) | normalized | denormalized | saturate); // sign : normalized : denormalized : saturate
        ...
        for (int i = 0, k = 0; i < image.width*image.height; i++)
        {
            switch (image.format)
            {
                ...
                case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1:
                {
                    ...
                    pixels[i].x = (float)((pixel & 0xf800) >> 11)*(1.0f/31);
                    pixels[i].y = (float)((pixel & 0x07c0) >> 6)*(1.0f/31);
                    pixels[i].z = (float)((pixel & 0x003e) >> 1)*(1.0f/31);
                    pixels[i].w = ((pixel & 0x0001) == 0)? 0.0f : 1.0f;
                ...
                case PIXELFORMAT_UNCOMPRESSED_R5G6B5:
                {
                    ...
                    pixels[i].x = (float)((pixel & 0xf800) >> 11)*(1.0f/31);
                    pixels[i].y = (float)((pixel & 0x07e0) >> 5)*(1.0f/63);
                    pixels[i].z = (float)(pixel & 0x001f)*(1.0f/31);
                    pixels[i].w = 1.0f;
                ...
                case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4:
                {
                    ...
                    pixels[i].x = (float)((pixel & 0xf000) >> 12)*(1.0f/15);
                    pixels[i].y = (float)((pixel & 0x0f00) >> 8)*(1.0f/15);
                    pixels[i].z = (float)((pixel & 0x00f0) >> 4)*(1.0f/15);
                    pixels[i].w = (float)(pixel & 0x000f)*(1.0f/15);
...
// api.md compatibility aliases: renamed upstream
void ImageDraw(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint)
{
    ImageDrawImagePro(dst, src, srcRec, dstRec, (Vector2){ 0.0f, 0.0f }, 0.0f, tint);
}
