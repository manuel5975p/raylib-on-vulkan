...
*     - All drawing goes through the rlgl-compatible immediate-mode layer in rlvk.h (Vulkan)
*     - Vertex batch overflow is handled explicitly: every shape calls
*       rlCheckRenderBatchLimit() with its worst-case vertex count before emitting
*     - The default shapes texture is resolved lazily to rlGetTextureIdDefault()
*       (1x1 white) because the default texture only exists after rlvkInit()
...
#endif
#ifndef SUPPORT_SPLINE_MITERS
    // Join consecutive spline segments with mitered corners instead of independent quads
    #define SUPPORT_SPLINE_MITERS          0      // Disabled by default (raylib default)
#endif
#ifndef SUPPORT_SPLINE_SEGMENT_CAPS
    // Draw a circle cap at the end of every spline segment
    #define SUPPORT_SPLINE_SEGMENT_CAPS    0      // Disabled by default (raylib default)
#endif
...
//----------------------------------------------------------------------------------
// Texture used on shapes drawing, id == 0 means "not set yet": it is resolved lazily
// to the rlvk default 1x1 white texture on first use (the default texture id is only
// valid once rlvkInit() has run, so it can not be a static initializer)
static Texture2D texShapes = { 0, 1, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 };
...
void SetShapesTexture(Texture2D texture, Rectangle source)
{
    ...
    {
        texShapes = (Texture2D){ rlGetTextureIdDefault(), 1, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 };
...
{
    // Resolve default shapes texture on first use (rlvk default 1x1 white texture)
    if (texShapes.id == 0)
    {
        texShapes = (Texture2D){ rlGetTextureIdDefault(), 1, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 };
        texShapesRec = (Rectangle){ 0.0f, 0.0f, 1.0f, 1.0f };
...
{
    if ((points == NULL) || (pointCount < 2)) return; // Security check

    rlCheckRenderBatchLimit(2*(pointCount - 1));
...
void DrawLineDashed(Vector2 startPos, Vector2 endPos, int dashSize, int spaceSize, Color color)
{
    ...
    rlCheckRenderBatchLimit(2*((int)(lineLength/(float)(dashSize + spaceSize)) + 2));
...
void DrawCircleSector(Vector2 center, float radius, float startAngle, float endAngle, int segments, Color color)
{
    ...
    rlCheckRenderBatchLimit(4*(segments/2 + 1));
...
void DrawCircleSectorLines(Vector2 center, float radius, float startAngle, float endAngle, int segments, Color color)
{
    ...
    rlCheckRenderBatchLimit(4*segments + 4);
...
// NOTE: Thick outline is drawn as a ring centered on the circle radius
void DrawCircleLinesEx(Vector2 center, float radius, float thick, Color color)
{
    ...
    DrawRing(center, innerRadius, outerRadius, 0.0f, 360.0f, 0, color);
}
...
void DrawRectangleRounded(Rectangle rec, float roundness, int segments, Color color)
{
    ...
    */
    ...
    rlCheckRenderBatchLimit(6*(4*(segments + 1) + 5));
...
void DrawRectangleRoundedLinesEx(Rectangle rec, float roundness, int segments, float lineThick, Color color)
{
    ...
    */
    ...
    rlCheckRenderBatchLimit(6*(4*segments + 4));
...
// NOTE: Vertex/color must be provided in counter-clockwise order
...
void DrawTriangleFan(const Vector2 *points, int pointCount, Color color)
{
    ...
    {
        rlCheckRenderBatchLimit(4*(pointCount - 2));
...
{
    if ((points == NULL) || (pointCount < 4)) return;
...
// api.md compatibility alias: renamed upstream to GetSplinePointBezierQuadratic
