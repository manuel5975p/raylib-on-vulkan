// NOTE (raylib-on-vulkan): This shim is a Win32-only port of the POSIX dirent API.
// It sits in src/external/, which is on the compiler include path, so a plain
// #include <dirent.h> would resolve here on POSIX systems as well; forward to
// the real system header in that case.
#if !defined(_WIN32)
