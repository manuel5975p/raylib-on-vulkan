...
int main(void)
{
    ...
    skybox.materials[0].shader = LoadShader("resources/shaders/vk/skybox.vert.spv",
                                            "resources/shaders/vk/skybox.frag.spv");

    // NOTE: rlvk binds samplers at fixed descriptor slots (set 1, binding = material map index)
    // and only exposes 4 slots, so there is no "environmentMap" texture unit uniform and the
    // cubemap must live in a map slot < 4: MATERIAL_MAP_DIFFUSE (binding 0) is used below
    SetShaderValue(skybox.materials[0].shader, GetShaderLocation(skybox.materials[0].shader, "doGamma"), (int[1]){ useHDR? 1 : 0 }, SHADER_UNIFORM_INT);
    SetShaderValue(skybox.materials[0].shader, GetShaderLocation(skybox.materials[0].shader, "vflipped"), (int[1]){ useHDR? 1 : 0 }, SHADER_UNIFORM_INT);

    // Load cubemap shader and setup required shader locations
    Shader shdrCubemap = LoadShader("resources/shaders/vk/cubemap.vert.spv",
                                    "resources/shaders/vk/cubemap.frag.spv");

    // NOTE: see above, the panorama sampler is declared at set 1, binding 0 in the Vulkan shader
    ...
    if (useHDR)
    {
        ...
        skybox.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture = GenTextureCubemap(shdrCubemap, panorama, 1024, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8);
    ...
    else
    {
        ...
        skybox.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture = LoadTextureCubemap(image, CUBEMAP_LAYOUT_AUTO_DETECT);
        UnloadImage(image);
    }
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        if (IsFileDropped())
        {
            ...
            if (droppedFiles.count == 1)         // Only support one file dropped
            {
                if (IsFileExtension(droppedFiles.paths[0], ".png;.jpg;.hdr;.bmp;.tga"))
                {
                    ...
                    UnloadTexture(skybox.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture);
        ...
        BeginDrawing();
            ...
            BeginMode3D(camera);
                ...
                // NOTE: rlvk keeps backface culling disabled by default (rlgl enables it) and its
                // 2D batch geometry is culled away when it is on, so it is not re-enabled here
...
static TextureCubemap GenTextureCubemap(Shader shader, Texture2D panorama, int size, int format)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        // NOTE: rlvk has no rlLoadDrawCube() helper, the internal batch system is used instead
