...
int main(void)
{
    ...
    Shader shdrSpot = LoadShader("resources/shaders/vk/base.vert.spv", "resources/shaders/vk/spotlight.frag.spv");
    ...
    {
        // NOTE: raylib-on-vulkan reflects top-level UBO members only, the shader flattens
        // the upstream "Spot spots[]" struct array into parallel arrays
        spots[i].positionLoc = GetShaderLocation(shdrSpot, TextFormat("spotPos[%i]", i));
        spots[i].innerLoc = GetShaderLocation(shdrSpot, TextFormat("spotInner[%i]", i));
        spots[i].radiusLoc = GetShaderLocation(shdrSpot, TextFormat("spotRadius[%i]", i));
    }
