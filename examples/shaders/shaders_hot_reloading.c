...
int main(void)
{
    ...
    const char *fragShaderFileName = "resources/shaders/vk/reload.frag.spv";
    time_t fragShaderFileModTime = GetFileModTime(TextFormat(fragShaderFileName, GLSL_VERSION));

    // Load raymarching shader
    // NOTE: Defining 0 (NULL) for vertex shader forces usage of internal default vertex shader
    Shader shader = LoadShader("resources/shaders/vk/default.vert.spv", TextFormat(fragShaderFileName, GLSL_VERSION));
    ...
    while (!WindowShouldClose())            // Detect window close button or ESC key
    {
        ...
        if (shaderAutoReloading || (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)))
        {
            ...
            if (currentFragShaderModTime != fragShaderFileModTime)
            {
                ...
                Shader updatedShader = LoadShader("resources/shaders/vk/default.vert.spv", TextFormat(fragShaderFileName, GLSL_VERSION));
