...
int main(void)
{
    ...
    // Load the shader
    Shader shader = LoadShader("resources/shaders/vk/base.vert.spv",
                               "resources/shaders/vk/rounded_rectangle.frag.spv");

    // Create a rounded rectangle
    RoundedRectangle roundedRectangle = CreateRoundedRectangle(
        (Vector4){ 5.0f, 10.0f, 15.0f, 20.0f },     // Corner radius
        20.0f,                                      // Shadow radius
        (Vector2){ 0.0f, 5.0f },                    // Shadow offset (Y down: gl_FragCoord is top-left origin)
        0.95f,                                      // Shadow scale
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            // NOTE: gl_FragCoord is top-left origin in Vulkan, no Y flip needed
