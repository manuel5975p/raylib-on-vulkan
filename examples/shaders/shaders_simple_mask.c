...
int main(void)
{
    ...
    // Load the shader
    Shader shader = LoadShader("resources/shaders/vk/base.vert.spv", "resources/shaders/vk/mask.frag.spv");
    ...
    // Using MATERIAL_MAP_NORMAL as a spare slot to use for 2nd texture
    // NOTE: Don't use MATERIAL_MAP_IRRADIANCE, MATERIAL_MAP_PREFILTER or  MATERIAL_MAP_CUBEMAP as they are bound as cube maps
    // NOTE: rlvk has 4 texture slots, so the spare map index must stay below 4
    // (MATERIAL_MAP_NORMAL == 5 would make rlActiveTextureSlot() a no-op)
    Texture texMask = LoadTexture("resources/mask.png");
    model1.materials[0].maps[MATERIAL_MAP_NORMAL].texture = texMask;
    model2.materials[0].maps[MATERIAL_MAP_NORMAL].texture = texMask;
    shader.locs[SHADER_LOC_MAP_NORMAL] = GetShaderLocation(shader, "mask");
