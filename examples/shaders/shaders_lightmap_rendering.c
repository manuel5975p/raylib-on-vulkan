...
int main(void)
{
    ...
    // NOTE: rlvk has no VAO state, vertex buffers are bound by their Mesh.vboId slot
    // (5 = texcoords2, see rlvk.h), so no rlSetVertexAttribute()/rlEnableVertexAttribute()
    mesh.vboId[5] = rlLoadVertexBuffer(mesh.texcoords2, mesh.vertexCount*2*sizeof(float), false);

    // Load lightmap shader
    Shader shader = LoadShader("resources/shaders/vk/lightmap.vert.spv",
                               "resources/shaders/vk/lightmap.frag.spv");
