...
// Port of glsl330/hybrid_raymarch.fs to the rayvulkan shader ABI
...
{
    float local_z = dot(normalize(ubo.camDir),rd)*Idist;
...
{
    vec3 horizontal = normalize(cross(ubo.camDir,vec3(.0 , 1.0, .0)));
    vec3 vertical   = normalize(cross(horizontal,ubo.camDir));
    return normalize(ubo.camDir + horizontal*nCoord.x + vertical*nCoord.y);
}

mat3 setCamera()
{
    vec3 cw = normalize(ubo.camDir);
...
void main()
{
    // gl_FragCoord has a top-left origin on Vulkan (GL had bottom-left): flip y
    vec2 fragCoord = vec2(gl_FragCoord.x, 2.0*ubo.screenCenter.y - gl_FragCoord.y);
    vec2 nCoord = (fragCoord - ubo.screenCenter.xy)/ubo.screenCenter.y;
    mat3 ca = setCamera();

    // focal length
    float fl = length(ubo.camDir);
    vec3 rd = ca*normalize(vec3(nCoord,fl));
    vec3 color = vec3(nCoord/2.0 + 0.5, 0.0);
    float depth = gl_FragCoord.z;
    {
        vec4 res = render(ubo.camPos - vec3(0.0, 0.0, 0.0) , rd);
        color = res.xyz;
