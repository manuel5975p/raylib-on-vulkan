...
int main(void)
{
    ...
    Shader shader = LoadShader("resources/shaders/vk/pbr.vert.spv",
                               "resources/shaders/vk/pbr.frag.spv");
...
static Light CreateLight(int type, Vector3 position, Vector3 target, Color color, float intensity, Shader shader)
{
    ...
    if (lightCount < MAX_LIGHTS)
    {
        ...
        light.enabledLoc = GetShaderLocation(shader, TextFormat("lightsEnabled[%i]", lightCount));
        light.typeLoc = GetShaderLocation(shader, TextFormat("lightsType[%i]", lightCount));
        light.positionLoc = GetShaderLocation(shader, TextFormat("lightsPosition[%i]", lightCount));
        light.targetLoc = GetShaderLocation(shader, TextFormat("lightsTarget[%i]", lightCount));
        light.colorLoc = GetShaderLocation(shader, TextFormat("lightsColor[%i]", lightCount));
        light.intensityLoc = GetShaderLocation(shader, TextFormat("lightsIntensity[%i]", lightCount));
