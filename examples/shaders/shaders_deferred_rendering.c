...
int main(void)
{
    ...
    Shader gbufferShader = LoadShader("resources/shaders/vk/gbuffer.vert.spv",
                               "resources/shaders/vk/gbuffer.frag.spv");

    Shader deferredShader = LoadShader("resources/shaders/vk/deferred_shading.vert.spv",
                               "resources/shaders/vk/deferred_shading.frag.spv");
    ...
    gBuffer.positionTextureId = rlLoadTexture(NULL, screenWidth, screenHeight, PIXELFORMAT_UNCOMPRESSED_R16G16B16, 1);
    ...
    gBuffer.normalTextureId = rlLoadTexture(NULL, screenWidth, screenHeight, PIXELFORMAT_UNCOMPRESSED_R16G16B16, 1);

    // Albedo (diffuse color) and specular strength can be combined into one texture
    // The color in RGB, and the specular strength in the alpha channel
    gBuffer.albedoSpecTextureId = rlLoadTexture(NULL, screenWidth, screenHeight, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 1);
    ...
    // NOTE: rlvk clamps the count to the attached color textures, so this comes after the attachments
    ...
    // NOTE: On rlvk the sampler2D uniforms gPosition/gNormal/gAlbedoSpec are bound by
    // their descriptor binding (set 1, binding 0/1/2), there are no GL texture units to set
                ...
                case DEFERRED_SHADING:
                {
                    BeginMode3D(camera);
                        ...
                        rlEnableShader(deferredShader.id);
                            ...
                            // NOTE: rlvk has no rlLoadDrawQuad(); the quad is emitted through the
                            // immediate-mode batch in clip space (the deferred VS ignores mvp)
                            rlSetTexture(gBuffer.positionTextureId);   // Keep slot 0 = gPosition
                            rlBegin(RL_QUADS);
                            ...
                            // Release the g-buffer textures from the sampler slots again:
                            // rlvk keeps bound textures until replaced, and they become
                            // color attachments again on the next geometry pass
                            rlActiveTextureSlot(texUnitPosition); rlDisableTexture();
                            rlActiveTextureSlot(texUnitNormal); rlDisableTexture();
                            rlActiveTextureSlot(texUnitAlbedoSpec); rlDisableTexture();
                    ...
                    // NOTE: rlBindFramebuffer() takes the GL target enums, not exported by rlvk.h,
                    // and rlBlitFramebuffer() on rlvk only blits color, so the depth copy is a no-op
                    rlBindFramebuffer(0x8CA8, gBuffer.framebufferId);   // GL_READ_FRAMEBUFFER
                    rlBindFramebuffer(0x8CA9, 0);                      // GL_DRAW_FRAMEBUFFER
